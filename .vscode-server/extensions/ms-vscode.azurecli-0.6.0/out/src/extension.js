...
class AzHoverProvider {
    ...
    provideHover(document, position, token) {
        ...
        const command = (0, parser_1.parse)(line);
        const list = command.subcommand;
        if (list.length && list[0].text === 'az') {
            const node = (0, parser_1.findNode)(command, position.character);
            if (node) {
...
class RunLineInEditor {
    ...
    run(source) {
        ...
        if (command.length > 0) {
            ...
            if (this.runningCommandCount === 1) {
                this.statusBarItemText = vscode_1.l10n.t('Azure CLI: Waiting for response');
            ...
            return this.findResultDocument()
                ...
                .then(target => replaceContent(target, JSON.stringify({ [vscode_1.l10n.t('Running command')]: command }) + '\n')
                .then(() => (0, utils_1.exec)(command))
    ...
    getSelectedCommand(source) {
        ...
        if (source.selection.isEmpty) {
            ...
            if (source.document.lineAt(lineNumber).text.length === 0) {
                vscode_1.window.showInformationMessage(vscode_1.l10n.t("Please put the cursor on a line that contains a command (or part of a command)."));
                return "";
            }
            ...
            if (selectionStart.line === selectionEnd.line) {
                ...
                for (let index = selectionStart.line + 1; index <= selectionEnd.line; index++) {
                    ...
                    if (line.trim().toLowerCase().startsWith(commandPrefix)) {
                        vscode_1.window.showErrorMessage(vscode_1.l10n.t("Multiple command selection not supported"));
                        return "";
                    }
    ...
    commandFinished(startTime) {
        this.runningCommandCount -= 1;
        this.statusBarItemText = vscode_1.l10n.t('Azure CLI: Executed in {0} milliseconds', Date.now() - startTime);
...
function azNotFound(wrongVersion) {
    return __awaiter(this, void 0, void 0, function* () {
        const message = wrongVersion
            ? vscode_1.l10n.t("'az' >= 2.0.5 required, please update your installation.")
            : vscode_1.l10n.t("'az' not found on PATH, please make sure it is installed.");
        const result = yield vscode_1.window.showInformationMessage(message, {
            title: vscode_1.l10n.t('Documentation'),
            run: installAzureCLI
        });
